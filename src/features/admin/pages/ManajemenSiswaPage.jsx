...
import { 
  ...
  UserPlus, X, Save, Edit3, Filter, ArrowUpCircle 
} from 'lucide-react';
...
const ManajemenSiswa = () => {
  ...
  const handleImportSiswa = async (e) => {
    ...
    if (!file.name.toLowerCase().endsWith('.xlsx')) {
      Swal.fire('Format tidak didukung', 'Gunakan file .xlsx untuk import siswa.', 'warning');
      return;
    }
    setIsImporting(true);
    try {
  ...
  return (
    <PageContainer className="max-w-6xl">
      ...
      <PageHeader className="mb-10 items-end">
        <div>
          <PageTitle className="text-4xl italic uppercase">Database Siswa</PageTitle>
          <PageSubtitle className="mt-2 italic">Data Management System</PageSubtitle>
        </div>
        
        <div className="flex flex-wrap gap-2">
          {/* TOMBOL EXPORT BARU */}
          <Button onClick={handleExportSiswa} className="text-[10px] uppercase">
          ...
          <label className="bg-blue-600 text-white px-5 py-3 rounded-2xl flex items-center gap-2 font-black text-[10px] uppercase shadow-lg shadow-blue-200 cursor-pointer">
            ...
            Import
            <input type="file" className="hidden" accept=".xlsx" onChange={handleImportSiswa} />
          </label>
        </div>
      </PageHeader>
      ...
      {isAddingManual && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className={`${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-100'} w-full max-w-md rounded-[40px] p-8 shadow-2xl animate-in zoom-in duration-300 border`}>
            ...
            <div className="space-y-4 text-left">
              <div>
                ...
                <input type="text" className={`${isDark ? 'bg-slate-700' : 'bg-gray-100'} w-full p-4 rounded-2xl mt-1 outline-none font-bold text-xs`} value={formData.nis} onChange={e => setFormData({...formData, nis: e.target.value})} placeholder="Nomor Induk Siswa" />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase text-gray-400 ml-2">Nama Lengkap</label>
                <input type="text" className={`${isDark ? 'bg-slate-700' : 'bg-gray-100'} w-full p-4 rounded-2xl mt-1 outline-none font-bold text-xs uppercase`} value={formData.nama_siswa} onChange={e => setFormData({...formData, nama_siswa: e.target.value})} placeholder="NAMA LENGKAP" />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase text-gray-400 ml-2">Kelas</label>
                <select className={`${isDark ? 'bg-slate-700' : 'bg-blue-50 text-blue-600'} w-full p-4 rounded-2xl mt-1 outline-none font-black text-xs uppercase cursor-pointer`} value={formData.kelas_id} onChange={e => setFormData({...formData, kelas_id: e.target.value})}>
      ...
      <div className="max-w-6xl mx-auto mb-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className={`md:col-span-2 p-2 rounded-[25px] shadow-sm border flex items-center px-6 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-100'}`}>
        ...
        <div className={`p-2 rounded-[25px] shadow-sm border flex items-center px-4 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-100'}`}>
      ...
      {/* TABLE */}
      <div className={`max-w-6xl mx-auto rounded-[40px] shadow-sm border overflow-hidden ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-100'}`}>
        <div className="overflow-x-auto">
          <table className="premium-table text-left">
            <thead className={`${isDark ? 'bg-slate-700/50' : 'bg-gray-50/50'} text-[10px] font-black text-gray-400 uppercase tracking-widest`}>
              <tr>
                <th>NIS</th>
                <th>Nama Siswa</th>
                <th>Kelas</th>

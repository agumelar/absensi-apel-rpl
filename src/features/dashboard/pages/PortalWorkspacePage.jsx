...
import { DASHBOARD_ROUTE, MAPEL_DASHBOARD_ROUTE, PEMBIASAAN_DASHBOARD_ROUTE } from '../../../shared/constants/routes';

const PortalWorkspacePage = ({ canAccessMapel, canAccessPembiasaanWorkspace, apelWorkspaceRoute = DASHBOARD_ROUTE }) => {
  return (
    <section className="mx-auto flex min-h-screen w-full max-w-5xl items-center px-4 py-8 md:px-8">
      <div className="w-full rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-sm backdrop-blur md:p-8">
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <Link
            to={canAccessPembiasaanWorkspace ? PEMBIASAAN_DASHBOARD_ROUTE : DASHBOARD_ROUTE}
            className="rounded-2xl border border-emerald-100 bg-white p-5 shadow-sm transition hover:shadow-md"
          >
            <h2 className="text-lg font-extrabold text-emerald-600">Pembiasaan</h2>
            <p className="mt-1 text-sm text-gray-500">
              {canAccessPembiasaanWorkspace
                ? 'Dashboard, Sapa Pagi, dan Kegiatan Pembiasaan Harian.'
                : 'Akses pembiasaan belum aktif untuk akun ini.'}
            </p>
          </Link>
        </div>
      </div>

...
import { 
  ...
  Calendar, Download, Building, ArrowRight, Search
} from 'lucide-react';
...
const ExecutiveDashboard = ({ user }) => {
  ...
  const exportExcelAkumulasi = () => {
    const dataExcel = filteredDataAkumulasi.map(d => ({ "Nama Siswa": d.nama, "Kelas": d.kelas, "H": d.hadir, "S": d.sakit, "I": d.izin, "A": d.alpha, "T": d.telat, "Total": d.total }));
    const ws = XLSX.utils.json_to_sheet(dataExcel);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Rekap");
    XLSX.writeFile(wb, `Rekap_Export_${getTodayDateWIB()}.xlsx`);
  };
          ...
          <div className="bg-white/5 p-8 rounded-[45px] border border-white/10 shadow-xl animate-in fade-in duration-500 text-left">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6 text-left">
               ...
               <div className="flex-1 max-w-sm w-full">
                  <div className="relative flex items-center bg-white/10 rounded-2xl border border-white/10 px-4 py-2 group focus-within:border-blue-500 transition-all">
                    <Search size={14} className="text-gray-500 group-focus-within:text-blue-500" />
                    <input type="text" placeholder="Cari nama siswa..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-transparent outline-none w-full ml-3 text-[10px] font-black uppercase text-white placeholder:text-gray-600" />
                  </div>
               </div>
               
               <div className="flex flex-wrap items-center gap-2">
                  <select value={modeAkumulasi} onChange={(e) => setModeAkumulasi(e.target.value)} className="bg-white/10 px-4 py-2 rounded-xl text-[9px] font-black uppercase outline-none text-white border border-white/10 cursor-pointer">
                    <option value="bulan" className="text-black">BULAN INI</option>
                    <option value="hari" className="text-black">HARI INI</option>
                    <option value="custom" className="text-black">POSTING DATE</option>
                  </select>
                  {modeAkumulasi === 'bulan' && (
                    <select value={filterBulan} onChange={(e) => setFilterBulan(parseInt(e.target.value))} className="bg-white/10 px-4 py-2 rounded-xl text-[9px] font-black uppercase outline-none text-white border border-white/10 cursor-pointer">
                       {["Jan", "Feb", "Mar", "Apr", "Mei", "Jun", "Jul", "Agu", "Sep", "Okt", "Nov", "Des"].map((m, i) => <option key={i} value={i+1} className="text-black">{m}</option>)}
                    </select>
                  )}
                  {modeAkumulasi === 'custom' && (
                    <div className="flex items-center gap-2 bg-white/10 px-3 py-2 rounded-xl border border-white/10">
                      <input type="date" value={dariTgl} onChange={(e) => setDariTgl(e.target.value)} className="bg-transparent outline-none text-[9px] font-black text-white" />
                      <ArrowRight size={12} className="opacity-30" />
                      <input type="date" value={sampaiTgl} onChange={(e) => setSampaiTgl(e.target.value)} className="bg-transparent outline-none text-[9px] font-black text-white" />
                    </div>
                  )}
            ...
            <div className="overflow-x-auto">
               <table className="w-full text-left">
                 <thead className="text-gray-500 uppercase border-b border-white/10 text-[9px] font-black tracking-widest italic text-left">
                   <tr>
                     <th className="pb-4">Nama Siswa</th>
                     <th className="pb-4 text-center">Kelas</th>
                     <th className="pb-4 text-center text-green-500">H</th>
                     <th className="pb-4 text-center text-orange-500">S</th>
                     <th className="pb-4 text-center text-blue-500">I</th>
                     <th className="pb-4 text-center text-red-500">A</th>
                     <th className="pb-4 text-center text-yellow-500 italic">T</th>
                     <th className="pb-4 text-center">Total</th>
                   </tr>
                 </thead>
                 <tbody className="text-[11px] font-bold text-left uppercase">
                   {filteredDataAkumulasi.length === 0 ? (
                     <tr><td colSpan="8" className="py-20 text-center opacity-30 italic font-black text-xs">Data tidak ditemukan 🌿</td></tr>
                   ) : filteredDataAkumulasi.map((r, i) => (
                     <tr key={i} className="border-b border-white/5 hover:bg-white/10 transition-all">
                       <td className="py-4 text-left uppercase">{r.nama}</td>
                       <td className="text-center opacity-40 font-black">{r.kelas}</td>
                       <td className="text-center text-green-500">{r.hadir}</td>
                       <td className="text-center text-orange-500">{r.sakit}</td>
                       <td className="text-center text-blue-500">{r.izin}</td>
                       <td className="text-center text-red-500">{r.alpha}</td>
                       <td className="text-center text-yellow-500 italic">{r.telat}</td>
